'ﾌｧｲﾙ名：xxCM01B0.frm ＜2019/12/25 マイグレ依頼ソース＞
'説　明：CFキャリア一覧／TFT/CFロット紐付き情報(無機ODF対応版)　メインフォーム
'作成日：2005/05/17 (Tue) 10:03:34 N.Kasai
'更新日：2019/06/10 (Mon) 09:45:44 T.Oide
'備　考：
'　　　：★作業開始、TPAL貼り合わせ登録からの起動の場合は「CFｷｬﾘｱ一覧」
'　　　：★装置別ﾛｯﾄ一覧、装置ｸﾞﾙｰﾌﾟ別ﾛｯﾄ一覧、工程別ﾛｯﾄ一覧からの起動の場合は「TFT/CFﾛｯﾄ紐付き情報」
'      ：
'      ：CFキャリア一覧／TFT/CFロット紐付き情報画面→作業開始画面→CFキャリア一覧／TFT/CFロット紐付き情報画面
'      ：と起動されることがあるため、TFT/CFロット紐付き情報画面の機能を別画面として起こす。
'      ：尚、時間的問題からxxCM00T0.frmより全機能を移植。(2014/11/05)
'Copyright(C)2003-2019, SEIKO EPSON CORPORATION.
Option Explicit On
Imports C1.Win.C1FlexGrid
Imports System.ComponentModel
Imports System.Security.Permissions
Public Class frmxxCM01B0
    '***************************************************************************************
    '                              * Shared変数の記述 *
    '***************************************************************************************
    '======================================Private==========================================
    ' NSYS 追加
    Private Shared _instance        As frmxxCM01B0    ' ただ一つのフォームのインスタンスを保持する変数
    ...
    '更新日：2018/12/05 (Wed)
    '備　考：
    Public Shared Property Instance() As frmxxCM01B0
        Get
            '_instanceがNothingまたは破棄されているときは、
            '新しくインスタンスを作成する
            If _instance Is Nothing OrElse _instance.IsDisposed Then
                _instance = New frmxxCM01B0
            End If
            Return _instance
        End Get
        Set(ByVal value As frmxxCM01B0)
    ...
    '@機能ID
    Private Const CMstrLocalMenuKey                     As String = CPstrKeyCM01B0          'ﾛｰｶﾙ機能ID

    '@CFキャリア一覧の場合のｸﾞﾘｯﾄﾞ項目設定
    Private Const CMvsfLotCols                          As Integer = 8                         'CFキャリア一覧の場合のｶﾗﾑ数
    ...
    '@vsfLotListの定数宣言(表示幅)
    Private Const CMvsfLotColWNo                        As Integer =  33  '500                  '№
    Private Const CMvsfLotColWCarrierID                 As Integer = 117 '1750                  'ｷｬﾘｱID
    Private Const CMvsfLotColWLotID                     As Integer = 137 '2050                  'ﾛｯﾄID
    Private Const CMvsfLotColWFlowClass                 As Integer =  42  '630                  '種別
    Private Const CMvsfLotColWPdID                      As Integer =  77 '1160                  '機種
    Private Const CMvsfLotColWWfNum                     As Integer =  76 '1140                  'WF枚数
    Private Const CMvsfLotColWChipNum                   As Integer =  76 '1140                  'ﾁｯﾌﾟ
    Private Const CMvsfLotColWPriority                  As Integer =  42  '630                  '優先度
    ...
    '@ｸﾞﾘｯﾄﾞの幅設定
    Private Const CMlngvsfTftCfBatchClassW              As Integer =  91 '1365         '装置種別
    Private Const CMlngvsfTftCfWPIDW                    As Integer =  42 '630          '号機
    Private Const CMlngvsfTftCfLOTIDW                   As Integer = 120 '1800         'ﾛｯﾄID
    Private Const CMlngvsfTftCfWFIDW                    As Integer =  90 '1350         'WFID
    Private Const CMlngvsfTftCfTPLOT1W                  As Integer = 120 '1800         'TPロット1
    Private Const CMlngvsfTftCfTPLOT2W                  As Integer = 120 '1800         'TPロット2
    Private Const CMlngvsfTftCfChipNumW                 As Integer =  60 '900          'チップ
    Private Const CMlngvsfTftCfStepIDW                  As Integer = 179 '2685         '小工程
    Private Const CMlngvsfTftCfBatchIDW                 As Integer =  80 '1140         'ﾊﾞｯﾁID
    Private Const CMlngvsfTftCfTpalClassW               As Integer =  97 '1455         '貼合制限
    ...
    Private Const CMvsfLotListHdHeight                  As Integer = 27 '400                   '行の高さ(ﾍｯﾀﾞｰのみ)
    Private Const CMvsfLotListHeight                    As Integer = 43 '640                   '行の高さ
    ...
    '@ﾚｽﾎﾟﾝｽ用定数
    Private Const CMstrFormName                         As String = "frmxxCM01B0"           '自ﾌｫｰﾑ名
    ...
    '更新日：2014/12/02 (Tue) 13:33:17 H.Hayashi
    '備　考：
    ...
    Private Sub cmdHyoumen_Click(ByVal sender As Object, ByVal e As EventArgs) Handles cmdHyoumen.Click
        ...
        Dim lstrBeforeLeftCol As Integer
        ...
        Try
            ...
            '再表示前の左位置を退避
            lstrBeforeLeftCol = vsfLotList.LeftCol
            ...
            'NSYS 行選択解除
            vsfLotList.Row = 0
            vsfLotList.LeftCol = lstrBeforeLeftCol
    ...
    '更新日：2014/12/02 (Tue) 13:34:52 H.Hayashi
    '備　考：
    '　　　：2009/10/02 (Fri) 16:58:36 N.Kojima     TFT/CFﾛｯﾄ紐付き情報表示機能追加に伴う修正。(案件№03791)
    '　　　：2014/12/02 (Tue) 13:31:10 H.Hayashi    組立無機ODF環境のｼｽﾃﾑ環境整備
    ...
    '更新日：2014/12/02 (Tue) 13:36:13 H.Hayashi
    '備　考：
    '　　　：2009/10/02 (Fri) 16:58:36 N.Kojima     TFT/CFﾛｯﾄ紐付き情報表示機能追加に伴う修正。(案件№03791)
    '　　　：2014/12/02 (Tue) 13:31:10 H.Hayashi    組立無機ODF環境のｼｽﾃﾑ環境整備
    ...
    '更新日：2014/12/02 (Tue) 13:36:34 H.Hayashi
    '備　考：
    ...
    Private Sub cmdNowList_Click(ByVal sender As Object, ByVal e As EventArgs) Handles cmdNowList.Click
        ...
        Try
            ...
            If plngfrmxxCM01B0Kbn = CPlngNumOne Then
    ...
    Private Sub vsfLotList_AfterSort(ByVal sender As Object, ByVal e As SortColEventArgs) Handles vsfLotList.AfterSort
        ...
        Try
            ...
            'NSYS 列未選択時に、ソート後の選択列がVB6版と異なるため、処理を追加
            vsfLotList.Select(vsfLotList.Row, e.Col)
    ...
    Private Sub vsfLotList_BeforeRowColChange(ByVal sender As Object, ByVal e As RangeEventArgs) Handles vsfLotList.BeforeRowColChange
        ...
        Try
            ...
            If e.OldRange.r1 <> e.NewRange.r1 And e.NewRange.r1 > 0 Then
                ...
                '@ｶﾚﾝﾄ行検索ｷｰを格納(№)
                If Not IsNothing(vsfLotList.GetData(e.NewRange.r1, CMvsfLotColNo)) Then
    ...
    '更新日：2014/12/02 (Tue) 13:37:01 H.Hayashi
    '備　考：
    '　　　：2009/10/02 (Fri) 16:58:36 N.Kojima     TFT/CFﾛｯﾄ紐付き情報表示機能追加に伴う修正。(案件№03791)
    '　　　：2014/12/02 (Tue) 13:31:10 H.Hayashi    組立無機ODF環境のｼｽﾃﾑ環境整備
    Private Sub vsfLotList_DblClick(ByVal sender As Object, ByVal e As EventArgs) Handles vsfLotList.DoubleClick
        ...
        '@↓2014/11/11 (Tue) 17:31:11 H.Hayashi **************************************************
        '   If pstrVaFlag = CPstrOne Or pstrTpalClass <> vbNullString Then
                
                Exit Sub
        '   End If
        '@↑2014/11/11 (Tue) 17:31:17 H.Hayashi **************************************************
    ...
    '更新日：2014/12/02 (Tue) 13:37:59 H.Hayashi
    '備　考：
    '　　　：2009/10/02 (Fri) 16:58:36 N.Kojima     TFT/CFﾛｯﾄ紐付き情報表示機能追加に伴う修正。(案件№03791)
    '　　　：2014/12/02 (Tue) 13:31:10 H.Hayashi    組立無機ODF環境のｼｽﾃﾑ環境整備
    ...
    '更新日：2014/12/02 (Tue) 13:38:06 H.Hayashi
    '備　考：
    '　　　：2009/10/02 (Fri) 16:58:36 N.Kojima     TFT/CFﾛｯﾄ紐付き情報表示機能追加に伴う修正。(案件№03791)
    '　　　：2014/12/02 (Tue) 13:31:10 H.Hayashi    組立無機ODF環境のｼｽﾃﾑ環境整備
    ...
    '***************************************************************************************
    '====================================Private============================================
    '関数名：prvFrmxxCM01B0_Init
    '機　能：画面情報の初期化処理(起動元機能によっては引継ぎ情報設定処理も有り)
    '引　数：なし
    '戻り値：なし
    '作成日：2005/05/19 (Thu) 09:33:17 N.Kasai
    '更新日：2014/11/11 (Tue) 15:33:36 H.Hayashi
    '備　考：
    '　　　：2009/10/02 (Fri) 16:58:36 N.Kojima     TFT/CFﾛｯﾄ紐付き情報表示機能追加に伴う修正。(案件№03791)
    '　　　：2014/12/02 (Tue) 13:31:10 H.Hayashi    組立無機ODF環境のｼｽﾃﾑ環境整備
    Private Sub prvFrmxxCM01B0_Init()
        ...
        Try
            ...
            '@ﾌｫｰﾑ起動区分が"1：TFT/CFﾛｯﾄ紐付き情報起動"か
            If plngfrmxxCM01B0Kbn = 1 Then

                '@画面のﾌｫｰﾑｷｬﾌﾟｼｮﾝに"TFT/CFﾛｯﾄ紐付き情報"を設定
                Me.Text = CPstrSubFormCM01B0
                Me.Text = CPstrSubFormCM01B0
        ...
        '@↓2019/06/06 (Thu) 10:38:14 Y.Yoneyama **************************************************
            '@作業開始は運用上使用しないので使用不可とする
            cmdWrkStart.Enabled = False
        '@↑2019/06/06 (Thu) 10:38:14 Y.Yoneyama **************************************************
        ...
        Catch ex As Exception
            ...
            With ptypOnErrorInfo
                .strMenuKey = CMstrLocalMenuKey
                .strProcName = "prvFrmxxCM01B0_Init"
                .strErrMessage = vbNullString
            End With
    ...
    '更新日：2014/12/02 (Tue) 13:39:39 H.Hayashi
    '備　考：
    '　　　：2009/10/02 (Fri) 16:58:36 N.Kojima     TFT/CFﾛｯﾄ紐付き情報表示機能追加に伴う修正。(案件№03791)
    '　　　：2013/03/14 (Thu) 10:24:07 T.Oide       TFT/CFﾛｯﾄ紐付き情報機能修正
    '　　　：2014/12/02 (Tue) 13:31:10 H.Hayashi    組立無機ODF環境のｼｽﾃﾑ環境整備
    Private Sub prvvsfLotList_Init()
        ...
        Try
            ...
            With vsfLotList
                ...
                .Styles.Normal.Trimming = StringTrimming.None  '省略符号(...)表示(文字列最後に表示)
                ...
                If plngfrmxxCM01B0Kbn = CPlngNumOne Then
                    ...
                    .SelectionMode = SelectionModeEnum.Row              '選択ﾓｰﾄﾞﾌﾘｰ
                Else
    ...
    '更新日：2014/11/05 (Wed) 09:39:46 H.Hayashi
    '備　考：
    '　　　：2014/11/05 (Wed) 09:39:46 H.Hayashi    組立無機ODFのシステム環境整備
    Private Sub prvGridBackColer(ByVal strCfFlag As String, ByVal strLpFlag As String, ByVal lngRowCnt As Integer)

        Try
            
            '@CF(TPAL)か
            If strCfFlag = 1 And strLpFlag = 0 Then
                '@背景色を灰色に変更
    ...
    '更新日：2014/11/05 (Wed) 09:43:40 H.Hayashi
    '備　考：
    '　　　：2014/11/05 (Wed) 09:39:46 H.Hayashi    組立無機ODFのシステム環境整備
                            ...
                            If lstrDispClass = .strEqType Or _
                               lstrDispClass = vbNullString Then
                                ...
                                With .typLotList(llngCnt2)
                                    ...
                                    '@TFT又はODFか
                                    If .strCfFlag = 0 Or .strLpFlag = 1 Then
                                        '@WF_ID(#01、#02の形式で表示する)
                                    ...
                                    '@背景色を灰色に変更
                                    Call prvGridBackColer(.strCfFlag, .strLpFlag, llngRowCnt)
    ...
    '更新日：2014/12/02 (Tue) 13:31:10 H.Hayashi
    '備　考：
    '　　　：2009/10/02 (Fri) 16:58:36 N.Kojima     TFT/CFﾛｯﾄ紐付き情報表示機能追加に伴う修正。(案件№03791)
        ...
        Try
            ...
            With vsfLotList
                ...
                Else
                    ...
                    Do While .Rows.Count > llngCnt
                        ...
                        With mtypCFListAns
                            ...
                            If IsNumeric(.typCFList(llngCnt - 1).strChipNum) Then
                            vsfLotList.SetData(llngCnt, CMvsfLotColChipNum, _
                                            Format$(CInt(.typCFList(llngCnt - 1).strChipNum), CPstrCFKnmaFormat))     '数量(CHIP)
                            Else
                                vsfLotList.SetData(llngCnt, CMvsfLotColChipNum, .typCFList(llngCnt - 1).strChipNum)
                            End If
                ...
                Else
                    ...
                    'NSYS グリッド表示時に、上記処理により１番右の列が選択状態となり
                    '     表示範囲がVB6版と異なるため、１番左の列から表示する設定処理
                    vsfLotList.LeftCol = 0
    ...
    '関数名：cmdWrkStart_Click
    '機　能：[作業開始]ﾎﾞﾀﾝ　押下時処理
    '引　数：なし
    '戻り値：なし
    '作成日：2014/11/12 (Wed) 10:04:53 H.Hayashi
    '更新日：2014/11/12 (Wed) 10:04:53 H.Hayashi
    '備　考：
    Private Sub cmdWrkStart_Click(ByVal sender As Object, ByVal e As EventArgs) Handles cmdWrkStart.Click

        Dim lstrLotIdColCarrierId       As String   'ﾛｯﾄIDAndｷｬﾘｱID
        Dim lstrLotIdColCarrierIdList() As String   'ﾛｯﾄIDAndｷｬﾘｱID配列
        Dim lstrCarrierID               As String   'ｷｬﾘｱID
        Dim llngCnt                     As Integer  'ｱﾚｲｶｳﾝﾄ用

        '@ ﾛｯﾄIDAndｷｬﾘｱIDを取得
        lstrLotIdColCarrierId = vsfLotList.GetData(vsfLotList.Row, CMlngvsfTftCfLOTID)
        '@ ﾛｯﾄIDAndｷｬﾘｱIDを分割
        lstrLotIdColCarrierIdList = Split(lstrLotIdColCarrierId, " ")
      
        '@ ﾃﾞｰﾀ分ﾙｰﾌﾟ
        For llngCnt = 0 To lstrLotIdColCarrierIdList.Count -1

            '@ ｷｬﾘｱ位置
            If llngCnt = 1 Then
                lstrCarrierID = lstrLotIdColCarrierIdList(llngCnt)
            End If
        Next llngCnt
        
        '@ ｷｬﾘｱID未取得時
        If lstrCarrierID = "" Then
            '@表示ﾒｯｾｰｼﾞ変換
            '@["<TRM131W>$$キャリアが選択されていない為、作業開始出来ません。"」のﾒｯｾｰｼﾞ表示
            pstrDMsg = pubstrMsgReplace_Set(CPstrMsgWar0131)
            Call publngMsgBoxInfo(pstrDMsg, vbExclamation, Me.Text, True, 16)

        '@ ｷｬﾘｱID取得時
        Else
        
            '@ 作業開始画面へ渡すためｷｬﾘｱIDを設定
